import { View, ActivityIndicator } from 'react-native';
import React, { useEffect, useState } from 'react';
import Container from '../../components/Container';
import { COLORS } from '../../theme/colors';
import Selector from '../../components/Selector';
import api from '../../services';
import axios from 'axios';
import ViewTableComponent from './ViewTableComponent';

const ViewByMonthYear = () => {
  const [loading, setLoading] = useState(true);
  const [selectedTemplateValue, setSelectedTemplateValue] = useState<string | null>(null);
  const [selectedTemplateLabel, setSelectedTemplateLabel] = useState<string | null>(null);
  const [viewByMonthYearData, setViewByMonthYearData] = useState<any[] | null>(null);
  const [tableData, setTableData] = useState<any | null>(null);
  const [tableLoading, setTableLoading] = useState(false);  

  useEffect(() => {
    getViewByMonthYearData();
  }, []);

  const getViewByMonthYearData = async () => {
    try {
      setLoading(true);
      const { data, status } = await api.client.getsortByMYData();
      if (status === 200 && data) {
        setViewByMonthYearData(data?.TemplateList);
      }
    } catch (error) {
      console.error('Error fetching data:', error);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    if (selectedTemplateValue) {
      fetchTableData(selectedTemplateValue);
    }
  }, [selectedTemplateValue]);

  const fetchTableData = async (templateValue?: string) => {
    const url = `https://apireposrtsystem-gmadfrdfb0cphham.centralindia-01.azurewebsites.net/api/Client/sortByMY?template=${templateValue}`;
    const token =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..Uwm-tBtRXJl9n4w4EtaDp3p4o7bWX_TT8tkxrWg_g5c';
    const headers = {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
    };

    try {
      setTableLoading(true);
      const response = await axios.post(url, { template: templateValue }, { headers });

      if (response.data) {
        setTableData(formatTableData(response.data));
      }
    } catch (error) {
      console.error('Error fetching table data:', error);
    } finally {
      setTableLoading(false);
    }
  };

  // Format API response into structured table data
  const formatTableData = (data) => {
    if (!data) return null;

    const columns = [...data.templateBind[0]];
    const rowLabels = data.templateBind.slice(1).flat();

    const mtdData = [...data.monthList, data.totalCol];
    const ytdData = [...data.yearList, data.yearlyTotalCol];

    return {
      columns,
      rowLabels,
      mtdData,
      ytdData,
    };
  };

  const formattedData = Array.isArray(viewByMonthYearData)
    ? viewByMonthYearData.map(item => ({
        label: item.text,
        value: item.value,
      }))
    : [];

  const handleTemplateSelection = (selectedLabel: string) => {
    const selectedItem = formattedData.find(item => item.label === selectedLabel);
    if (selectedItem) {
      setSelectedTemplateLabel(selectedItem.label);
      setSelectedTemplateValue(selectedItem.value);
    }
  };

  return (
    <Container
      header={{
        title: `View By Month/Year`,
        backgroundColor: COLORS.primaryColor,
        statusBarType: 'dark-content',
      }}
    >
      <View style={{ padding: 16, paddingBottom: 5 }}>
        <Selector
          data={formattedData}
          onChangeText={handleTemplateSelection}
          value={selectedTemplateLabel || ''}
          type="label"
          placeholder="Select a Template *"
          containerStyle={{ width: '100%' }}
        />
      </View>

      {tableLoading ? (
        <ActivityIndicator size="large" color={COLORS.newDark} style={{flex:1}}/>
      ) : (
        tableData && (
          <>
            <ViewTableComponent title="MTD Data" columns={tableData.columns} rowLabels={tableData.rowLabels} data={tableData.mtdData} />
            <ViewTableComponent title="YTD Data" columns={tableData.columns} rowLabels={tableData.rowLabels} data={tableData.ytdData} />
          </>
        )
      )}
    </Container>
  );
};

export default ViewByMonthYear;
