...
const Dashboard = () => {
  ...
  const operational = data.secondData.slice(0, 5).reduce((sum, val) => sum + (val || 0), 0);

  // Compute Net Flow (sum of Fixed Asset, Loans, Investment, Non Operational)
  const netFlow = data.thirdData.slice(0, 4).reduce((sum, val) => sum + (val || 0), 0);
